			...
			<div class = "left">
				...
				<c:choose>
					<c:when test = "${not empty instruments}">
						<b><fmt:message key = "instruments.found" /> ${instrumentsNumber}</b>
					</c:when>
					<c:otherwise>
				...
				<form action = "controller" style = "margin: 0" method = "post" id = "filterForm">
					...
					<c:forEach var = "field" items = "${fields}">
						...
						<select name = "field_${field.id}" style = "width: 95%">
							<option value = "empty">
								<fmt:message key = "any_value" />
							</option>
							<c:forEach var = "fieldValue" items = "${field.existingValues}">
								<c:forEach var = "filterField" items = "${filterFields}">
									<c:if test = "${field.id eq filterField.id}">
										<c:set var = "filterValue" value = "${filterField.currentValue}" />
									</c:if>
								</c:forEach>
								<option value = "value_${fieldValue.id}" 
									<c:if test = "${fieldValue.id eq filterValue.id}">
										selected
									</c:if>
								>
									<c:choose>
								...
								<c:choose>
									<c:when test = "${page == pageNumber}">
										<li class = "page-item" >
											...
											href = "controller?command=instruments_subgroup_filter_page&page_number=${page}&subgroupId=${subgroup.id}">
												${page}
											</a>
										</li>
									</c:when>
